...
phi = zeros(L,2*L,N_comp);
% phi(floor(nx/2),1) = 1;
% phi(:,1:L,:) = epsilon0;
phi(:,1:L,:) = epsilon0*(2*rand(L,L,N_comp)-1);
% phi(:,1:L,:) = epsilon0*(1+0.5*randn(L,L,N_comp));
% phi(:,1:L,2) = -epsilon0;
% phi(:,1:L,:) = epsilon0*(rand(L,L,L,N_comp));
order_total = zeros(1,nt);
order = zeros(N_comp,nt);
% order(1) = sum(sqrt(phi(1,:,:,:).^2 + phi(2,:,:,:).^2),'all');
order(:,1) = sum(phi(:,1:L,:),1:2);
